...
void Graphics::Create_Planes(const Grid &g)
{
    ...
    for (int i = 1; i <= nrow; i++)
    {
        for (int j = 1; j <= ncol; j++)
        {
            ...
            fields_display[1][i][j] = min(206, 3 + (int)(smallmagnification * g.metfield[i][j][emd.at("glc_D")])); // glucose
...
void Graphics::Write_Movie(const Grid &g)
{
    ...
    for (int i = 1; i <= nrow; i++)
    {
        for (int j = 1; j <= ncol; j++)
        {
            int whitespecies = fields_display[21][i][j] + fields_display[22][i][j] + fields_display[23][i][j] + fields_display[24][i][j] +
                               fields_display[30][i][j] + fields_display[31][i][j] + fields_display[34][i][j] + fields_display[35][i][j] + fields_display[36][i][j] + fields_display[37][i][j]; 

            int redspecies = fields_display[27][i][j] + fields_display[28][i][j] + fields_display[29][i][j]; // Bifidobacteria
            int greenspecies = fields_display[26][i][j];                                                     // B. hansenii
            int orangespecies = fields_display[25][i][j];                                                    // Enterobacter cloacea
            int bluespecies = fields_display[33][i][j];                                                      // E. coli
            int pinkspecies = fields_display[32][i][j];                                                      // Bacteroides vulgatus
            ...
            image.plot(j, 8 * nrow + 8 + i, 0, 0, min(250 * fields_display[7][i][j], 65534));                                                                                      // ethanol
            image.plot(j, 7 * nrow + 7 + i, 0, 0, min(250 * fields_display[15][i][j], 65534));                                                                                     // formate
            ...
            image.plot(j, 2 * nrow + 2 + i, 0, 0, 0);                                                                                                                              // empty
            image.plot(j, nrow + 1 + i, 0, 0, 0);                                                                                                                                  // empty
