...
int main(int argc, char *argv[])
{
    ...
    par.foodinglucose = atof(argv[4]);
    par.foodingalactose = atof(argv[5]);
    ...
    // All in vitro species
    AGORAParam bp1("MODEL_Pdis.xml", 1, par.fluxweight, gibbsmap);
    AGORAParam bp2("MODEL_Caer.xml", 2, par.fluxweight, gibbsmap);
    AGORAParam bp3("MODEL_Rgna.xml", 3, par.fluxweight, gibbsmap);
    AGORAParam bp4("MODEL_Vdis.xml", 4, par.fluxweight, gibbsmap);
    AGORAParam bp5("MODEL_Eclo.xml", 5, par.fluxweight, gibbsmap);
    AGORAParam bp6("MODEL_Bhan.xml", 6, par.fluxweight, gibbsmap);
    AGORAParam bp7("MODEL_BlongLong.xml", 7, par.fluxweight, gibbsmap);  
    AGORAParam bp8("MODEL_Bbre.xml", 8, par.fluxweight, gibbsmap);
    AGORAParam bp9("MODEL_BlongInf.xml", 9, par.fluxweight, gibbsmap);
    AGORAParam bp10("MODEL_Lgas.xml", 10, par.fluxweight, gibbsmap);
    AGORAParam bp11("MODEL_Efae.xml", 11, par.fluxweight, gibbsmap);
    AGORAParam bp12("MODEL_Bvul.xml", 12, par.fluxweight, gibbsmap);
    AGORAParam bp13("MODEL_Ecol.xml", 13, par.fluxweight, gibbsmap);
    AGORAParam bp14("MODEL_Dfor.xml", 14, par.fluxweight, gibbsmap);
    AGORAParam bp15("MODEL_Ssal.xml", 15, par.fluxweight, gibbsmap);
    ...
    for (simultime = 1; simultime <= n_iterations; simultime++) // 1 h = 20 timesteps
    {
        ...
        if (par.metdrift < 1000 && par.metdrift > 0)
        {
            ...
            {
                if (simultime % (int)(par.timescale / par.metdrift) == 1)
                {
                    My_Grid.DriftCells(b_params); // Cells and metabolites all drift to the right
                }
            }
